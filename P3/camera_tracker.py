...
parser.add_argument("-v", "--video", help="Path for the detection video", required=False)
parser.add_argument("-c", "--calibration", help="Path for the calibration images directory", required=False)
parser.add_argument("-i", "--ip", help="URL for IP webcam", required=False)
args = parser.parse_args()


print("Camera calibration:")
if (args.video is None or args.calibration is None) and args.ip is None:
    if (args.video is None) ^ (args.calibration is None):
        ...
        args.video = 0
        print("Default camera will be used")
    calib_images = utils.default_camera_calibration_images(args.video)
elif args.ip is not None:
    calib_images = utils.default_camera_calibration_images(args.ip)
    args.video = args.ip
else:
...
while cap.isOpened():
    ...
    if ret:
        ...
        if aruco_corners:
            for i, tag_id in enumerate(ids):
                ...
                _, rotation, translation = cv2.solvePnP(object_points,
                                                        ...
                                                        intrinsics,
                                                        dist_coeffs)
                frame = cv2.drawFrameAxes(frame, intrinsics, dist_coeffs, rotation, translation, 100, thickness=2)
        ...
        wk = cv2.waitKey(24)
        if wk & 0xFF == ord('p'):
            cv2.imwrite("img/axis.jpg", frame)
            wk = cv2.waitKey(0)
        if wk & 0xFF == ord('q'):
